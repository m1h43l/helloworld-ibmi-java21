...
// This class is borrowed from the Helidon project: io.helidon.webserver.ThreadPerTaskExecutor
public class ThreadPerTaskExecutor implements HelidonTaskExecutor {
    ...
    public boolean terminate(long timeout, TimeUnit unit) {
        ...
        if (state.compareAndSet(RUNNING, SHUTDOWN)) {
            ...
            if (threadTasks.isEmpty()) { return state.compareAndSet(SHUTDOWN, TERMINATED); }
            try {
        ...
        if (!isTerminated()) {
            if (state.get() == RUNNING) {
                throw new IllegalArgumentException(
                        "Must call terminate(long, TimeUnit) first" + " to attempt graceful termination");
            }
    ...
    /**
     * Sends interrupt signal to all tasks that can be interrupted at this time and
     * returns that set.
     */
    private Set<Thread> tryStopInterruptableTasks() {
        return threadTasks.entrySet()
                .stream()
    ...
    private void taskComplete(Thread thread) {
        ...
        if (state.get() == SHUTDOWN && threadTasks.isEmpty()) { terminationSignal.countDown(); }
    }
    ...
    private void start(Thread thread, Object task) {
        ...
        try {
            ...
            if (!started) { taskComplete(thread); }
        }

        // throw REE if thread not started and no exception thrown
        if (!started) { throw new RejectedExecutionException(); }
    }
    ...
    /**
     * A Future for a task that runs in its own thread. The thread is created (but
     * not started) when the Future is created. The thread is interrupted when the
     * future is cancelled. The executor is notified when the task completes.
     */
    private static class ThreadBoundFuture<T> extends CompletableFuture<T> implements Runnable {
        ...
        public boolean cancel(boolean mayInterruptIfRunning) {
            ...
            if (cancelled && mayInterruptIfRunning) { thread.interrupt(); }
            return cancelled;
        }
    }
